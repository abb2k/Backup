...
BackupsLayer* BackupsLayer::create() {
  BackupsLayer* ret = new BackupsLayer();
...
bool BackupsLayer::init(float _w, float _h, const char* _spr) {
  ...
  CCSprite* CloseS = CCSprite::createWithSpriteFrameName("GJ_closeBtn_001.png");
  CloseS->setScale(GetFixedScale(1));

  CCMenuItemSpriteExtra* CloseButton = CCMenuItemSpriteExtra::create(CloseS, nullptr, this, menu_selector(BackupsLayer::backButtonCallback));
  ...
  addButton = CCMenuItemSpriteExtra::create(addBSprite, nullptr, this, menu_selector(BackupsLayer::addButtonClicked));
  addButton->setPosition(GetResFixedScale({164, 54}));
  m_buttonMenu->addChild(addButton);

  // add backup button
  CCSprite* deleteBSprite = CCSprite::createWithSpriteFrameName("GJ_deleteBtn_001.png");
  deleteBSprite->setScale(GetFixedScale(1));
  CCMenuItemSpriteExtra* deleteButton = CCMenuItemSpriteExtra::create(deleteBSprite, nullptr, this, menu_selector(BackupsLayer::deleteButtonClicked));
  deleteButton->setPosition(GetResFixedScale({164, -54}));
  m_buttonMenu->addChild(deleteButton);

  // create open folder button
  CCSprite* OFSprite = CCSprite::createWithSpriteFrameName("GJ_plainBtn_001.png");
  OFSprite->setScale(GetFixedScale(0.75f));
  CCSprite* OFSpriteFolder = CCSprite::createWithSpriteFrameName("folderIcon_001.png");
  OFSpriteFolder->setPosition(OFSprite->getContentSize() / 2);
  OFSpriteFolder->setScale(0.8f);
  OFSprite->addChild(OFSpriteFolder);

  CCMenuItemSpriteExtra* OFButton = CCMenuItemSpriteExtra::create(OFSprite, nullptr, this, menu_selector(BackupsLayer::OpenFolder));
  OFButton->setPosition(GetResFixedScale({212, -105}));
  m_buttonMenu->addChild(OFButton);

  // add settings button
  CCSprite* settingsBSprite = CCSprite::createWithSpriteFrameName("GJ_optionsBtn_001.png");
  settingsBSprite->setScale(GetFixedScale(0.7f));
  CCMenuItemSpriteExtra* settingsButton = CCMenuItemSpriteExtra::create(settingsBSprite, nullptr, this, menu_selector(BackupsLayer::openSettings));
  settingsButton->setPosition(GetResFixedScale({209, 104}));
  m_buttonMenu->addChild(settingsButton);

  // add mode button
  CCSprite* modeBSprite = CCSprite::createWithSpriteFrameName("GJ_plainBtn_001.png");
  ...
  CCMenuItemSpriteExtra* modeButton = CCMenuItemSpriteExtra::create(modeBSprite, nullptr, this, menu_selector(BackupsLayer::changeViewMode));
...
void BackupsLayer::backButtonCallback(CCObject* object) { keyBackClicked(); }

void BackupsLayer::addButtonClicked(CCObject* object) { CreateBackupLayer::create(this)->show(this); }

void BackupsLayer::deleteButtonClicked(CCObject* object) {
  deleteWarning = FLAlertLayer::create(this, "Warning!", "Are you sure you want to delete\nall selected backups?", "No", "Yes");
  deleteWarning->show();
}
  ...
  if (p0 == deleteWarning) {
    if (p1) {
      ...
      CCARRAY_FOREACH(list->m_listView->m_entries, obj) {
        BackupCell* cell = static_cast<BackupCell*>(obj);
        if (cell) {
          if (cell->selected)
            ghc::filesystem::remove_all(cell->_folderPath);
        }
      }
      RefreshBackupsList();
    }
...
void BackupsLayer::OpenFolder(CCObject* object) { file::openFolder(BackupsDir); }

void BackupsLayer::openSettings(CCObject* object) { geode::openSettingsPopup(Mod::get()); }
...
void BackupsLayer::importBackup(CCObject* object) {
  ...
  file::pickFile(file::PickMode::OpenFile, options, [this](ghc::filesystem::path path) {
    if (path.extension() == ".gdbackup") {
      matjson::Value fileRes = file::readJson(path).value();
      matjson::Object value = fileRes.as_object();
      gdbackupFile myfile = {
      ...
      FLAlertLayer::create("Import Failed!", "Please choose a \".gdbackup\" file", "OK")->show();
    }
  });
}
...
float BackupsLayer::GetFixedScale(float scale) { return scale / DefaultScaleMultiplier; }
