...
bool RenameBackupLayer::init(float _w, float _h, BackupsLayer* _parentLayer, BackupCell* cell, const char* _spr) {
  ...
  auto DoneButton = CCMenuItemSpriteExtra::create(DoneS, nullptr, this, menu_selector(RenameBackupLayer::DoneAndSave));
...
void RenameBackupLayer::OnExport(CCObject* object) {
  exportAlert = FLAlertLayer::create(this, "Export Backup", "Do you want to export\n \"" + backupCell->Name + "\"?", "No", "Yes");
  exportAlert->show();
}
...
void RenameBackupLayer::DoneAndSave(CCObject* object) {
  if (NameInput->getString() != "") {
    ...
    if (!file.fail()) {
      ...
      FLAlertLayer::create("Fail", "Backup Not Found.", "Back")->show();
    }
  } else {
...
void RenameBackupLayer::FLAlert_Clicked(FLAlertLayer* p0, bool p1) {
  if (exportAlert == p0 && p1) {
    ...
    FLAlertLayer::create("Exported!", backupCell->Name + " was exported to\n" + (Mod::get()->getSaveDir() / "Backups/Exports").string(), "OK")->show();
